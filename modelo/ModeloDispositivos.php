...
static function createCctv($datos){
    $conexion = Conexion::conectar();
    try{
        ...
        $statement = $conexion->prepare("CALL insertar_cctvs(?,?,?,?,?,?,?,?,?)");
        $statement -> bind_param("ssiisssss",
            $datos["modelo"],
            $datos["numero_serie"],
            $id_marca,
            $precio,
...
static function selectDispositivoTipo($tipoSeleccionado){
    try{
        ...
        $stmt = $conexion->prepare("CALL inventarit_manager.inventario_dispositivo(?)");
